...
users ─────────────────────────────────────────────────┐
  ...
  ├── reviews (1:N)                                     │
  ├── collections (1:N)                                 │
  ├── itineraries (1:N) ──── itinerary_days (1:N)       │
  │                              └── itinerary_items    │
  ├── ai_chat_sessions (1:N)                            │
  │       └── ai_chat_messages (1:N)                    │
  ├── trip_groups ──── group_members                    │
  └── community_posts                                   │
                                                        │
places ─────────────────────────────────────────────────┘
  │   category_id → categories
  ├── place_images (1:N)
  ├── place_reviews (1:N) ← users
  ├── place_embeddings (1:1)
  └── place_sources (1:N) [External data provenance]

categories (hierarchy: parent_id self-ref)

travel_areas [Vietnam regions/cities metadata]

data_coverage [Region crawl status]

crawl_jobs [Job tracking]

knowledge_documents
  └── knowledge_chunks (1:N) [RAG corpus]
```
...
**Vấn đề với schema cũ** (từ `DB.md`):

1. **`place_embeddings` (1 row / place)**: Tốt cho `embedding` nhưng `content_chunk TEXT` gợi ý muốn store nhiều chunks. Nếu một place có nhiều chunks (từng chunk = description, category context, review summary), cần thiết kế khác đi. **Đề xuất**: giữ 1 embedding per place với concatenated content cho semantic search. RAG dùng bảng riêng `knowledge_chunks`.

2. **`itinerary_destinations`**: Thiếu `itinerary_days` intermediate table. Flat structure với `day_number` INT là đủ cho đồ án, nhưng thêm `estimated_duration_minutes` quan trọng.

3. **Thiếu `place_sources`**: Không có cách track data provenance — không biết place đến từ OSM hay Google, không có `external_id`, không thể sync.

4. **Thiếu `crawl_jobs`**: Không thể track trạng thái job.

5. **`reviews.images TEXT[]`**: Nên là bảng riêng hoặc JSONB có metadata hơn.

6. **`categories` thiếu `parent_id`**: Không thể làm hierarchy (ví dụ: `Ăn uống > Nhà hàng > Hải sản`).

7. **`places.city` DEFAULT 'Hồ Chí Minh'**: Hard-code — thay bằng FK đến `travel_areas`.

---

### Schema đầy đủ (Production-oriented)
...
CREATE EXTENSION IF NOT EXISTS "pg_trgm"; -- Fuzzy text matching cho deduplication
...
CREATE TYPE budget_level_enum      AS ENUM ('BUDGET', 'MODERATE', 'UPSCALE', 'LUXURY');
...
-- 1:1 với users — profile du lịch của user
-- Lý do cần: AI Agent dùng preferences để personalize recommendation mà không cần join nặng
CREATE TABLE user_preferences (
    user_id             UUID PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
    budget_level        budget_level_enum DEFAULT 'MODERATE',
    preferred_categories TEXT[] DEFAULT '{}',  -- ['restaurant', 'beach', 'museum']
    dietary_restrictions TEXT[] DEFAULT '{}',  -- ['vegetarian', 'halal']
    travel_style        TEXT[] DEFAULT '{}',   -- ['adventure', 'relaxing', 'cultural']
    preferred_cities    TEXT[] DEFAULT '{}',
    mobility_notes      TEXT,                  -- wheelchair, elderly, etc.
    updated_at          TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);
...
-- Hierarchy categories: Ăn uống > Nhà hàng > Hải sản
-- Lý do cần parent_id: filter theo category tree (tất cả type ẩm thực)
...
-- Travel areas = Vietnam provinces/cities + tourist zones
-- Lý do cần: Data coverage check, crawl job targeting, bounding box cho spatial queries
CREATE TABLE travel_areas (
    ...
    -- Bounding box cho crawl region
    bbox_min_lat    DOUBLE PRECISION,
    bbox_max_lat    DOUBLE PRECISION,
    bbox_min_lng    DOUBLE PRECISION,
    bbox_max_lng    DOUBLE PRECISION,
    -- PostGIS boundary polygon (dùng cho ST_Within)
...
CREATE TABLE places (
    ...
    name_normalized VARCHAR(255),           -- lowercase, no diacritics (cho fuzzy match)
    description     TEXT,
    ...
    location        GEOGRAPHY(POINT, 4326),  -- Trigger auto-populate từ lat/lng

    -- Price (JSONB cho flexibility)
    -- { "min": 50000, "max": 200000, "currency": "VND", "level": "MODERATE" }
    price_range     JSONB,
    price_level     budget_level_enum,       -- Denormalized cho fast filter
    ...
    --   "tuesday": { ... },
    --   "special": [{ "date": "2024-01-01", "closed": true }]
    -- }
    phone           VARCHAR(30),
    website         TEXT,
    email           VARCHAR(255),

    -- Aggregate rating (computed/denormalized từ reviews)
    -- rating_avg update qua trigger hoặc background job
    rating_avg      FLOAT DEFAULT 0.0,
    review_count    INT DEFAULT 0,

    -- Tags & Attributes
    tags            TEXT[] DEFAULT '{}',     -- ['rooftop', 'pet-friendly', 'wifi']
    ...
    image_count     INT DEFAULT 0,           -- Denormalized
    has_description BOOLEAN GENERATED ALWAYS AS (description IS NOT NULL AND length(description) > 50) STORED,

    -- Lifecycle
    status          place_status_enum DEFAULT 'PENDING',
    created_by      UUID REFERENCES users(id) ON DELETE SET NULL,
    merged_into     UUID REFERENCES places(id) ON DELETE SET NULL, -- Nếu duplicate
    created_at      TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP,
    updated_at      TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP
);

-- Place images — separate table cho multi-image support
-- Lý do cần riêng: một place có nhiều ảnh, cần order, primary flag
...
-- External data sources per place — core cho multi-provider architecture
-- Lý do cần: track data từ đâu, external ID để sync, conflict resolution
CREATE TABLE place_sources (
    id                  UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    place_id            UUID NOT NULL REFERENCES places(id) ON DELETE CASCADE,
    provider            VARCHAR(50) NOT NULL,   -- 'osm', 'google_places', 'tripadvisor'
    external_id         VARCHAR(255) NOT NULL,  -- ID trên provider
    external_url        TEXT,
    -- Metadata từ provider (raw)
    raw_data            JSONB,
    -- Sync state
    ...
    -- Data quality từ source này
    source_rating       FLOAT,                  -- Rating từ provider này
    source_review_count INT,
    CONSTRAINT uq_provider_external UNIQUE(provider, external_id)
);

-- Place reviews — user-generated content
-- Lý do tách khỏi places: 1 place có nhiều reviews, cần separate queries
CREATE TABLE place_reviews (
    ...
    title       VARCHAR(255),
    comment     TEXT,
    visited_at  DATE,                           -- Ngày ghé thăm thực tế
...
| Bảng | Lý do |
|------|-------|
| `places` | Core entity — business data của địa điểm |
| `place_images` | 1 place có N ảnh, cần order và primary flag |
| `place_sources` | Multi-provider provenance, sync tracking, không bị lock vào 1 source |
| `place_reviews` | User-generated ratings, tách để query riêng, tính avg qua trigger |
| `place_embeddings` | Bảng riêng vì vector ~6KB, không cần load khi query thông thường |
| `categories` | Taxonomy có hierarchy (parent_id), tái sử dụng nhiều nơi |
| `travel_areas` | Metadata cho 63 tỉnh thành VN, bbox cho crawl targeting |
| `data_coverage` | Quick check "tỉnh X có đủ data?" mà không cần COUNT(*) mỗi lần |
| `crawl_jobs` | Admin visibility, checkpoint/resume, dead-letter tracking |
| `itineraries` | Container cho multi-day trip, lưu AI prompt và preferences |
| `itinerary_days` | Mỗi ngày có summary, cost, starting point riêng |
| `itinerary_items` | Items trong ngày: timing, cost, travel from previous |
| `user_preferences` | AI Agent dùng để personalize, 1:1 với user |
| `knowledge_documents` | Quản lý nguồn RAG corpus |
| `knowledge_chunks` | Unit retrieve trong RAG với embedding riêng |
| `ai_chat_sessions` | Conversation context, linked itinerary |
| `ai_chat_messages` | Full conversation history, tool call logging |
| `collections` | Saved places — user's favorites |
| `trip_groups` | Collaborative trip planning |

**Không tạo:**
- ❌ `travel_routes` riêng — route data lưu trong `itinerary_items.travel_from_previous` (JSONB)
- ❌ `recommendation_history` — có thể log ở Redis/external log
- ❌ `place_features` riêng — dùng `places.attributes` JSONB là đủ cho đồ án
